...
var sparkscorenode = require('..');
var utils = require('../utils');

function main(servicesPath, additionalServices) {
  /* jshint maxstatements: 100 */

  var version = sparkscorenode.version;
  var create = sparkscorenode.scaffold.create;
  var add = sparkscorenode.scaffold.add;
  var start = sparkscorenode.scaffold.start;
  var remove = sparkscorenode.scaffold.remove;
  var callMethod = sparkscorenode.scaffold.callMethod;
  var findConfig = sparkscorenode.scaffold.findConfig;
  var defaultConfig = sparkscorenode.scaffold.defaultConfig;
  ...
  program
    ...
    .option('-d, --datadir <dir>', 'Specify the sparks database directory')
  ...
  program
    ...
    .option('-c, --config <dir>', 'Specify the directory with sparkscore Node configuration')
    .option('-d, --daemon', 'Make sparkscore-node a daemon (running in the background)')
    .action(function(cmd){
      if (cmd.config) {
  ...
  program
    ...
    .action(function(services){
      ...
      if (!configInfo) {
        throw new Error('Could not find configuration, see `sparkscore-node create --help`');
      }
      var opts = {
      ...
      console.log();
      console.log('    $ sparkscore-node add wallet-service');
      console.log('    $ sparkscore-node add insight-api-sparks');
      console.log();
    });
  ...
  program
    ...
    .action(function(services){
      ...
      console.log('  Examples:');
      console.log();
      console.log('    $ sparkscore-node remove wallet-service');
      console.log('    $ sparkscore-node remove insight-api-sparks');
      console.log();
    });
