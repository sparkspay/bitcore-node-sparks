...
var SparkscoreNode = require('../node');
var index = require('../');
...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 'A reindex for sparksd is necessary for this upgrade with the "reindex=1" sparks.conf option. \n' +
                 'There are changes necessary in both sparks.conf and sparkscore-node.json. \n\n' +
                 'To upgrade please see the details below and documentation at: \n' +
                 'https://github.com/sparkspay/sparkscore-node/blob/sparksd/docs/upgrade.md \n');
        ...
        servicesConfig: {
          sparksd: {
            spawn: {
              ...
              exec: path.resolve(__dirname, '../../bin/sparksd')
            }
          }
        }
      };
 ...
 * @param {Object} options.config - The parsed sparkscore-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './sparkscore-node.json');
...
function loadModule(req, service) {
  try {
    // first try in the built-in sparkscore-node services directory
    ...
    if (servicePackage.sparkscoreNode) {
      serviceModule = service.name + '/' + servicePackage.sparkscoreNode;
    }
 ...
 * @param {Node} node - The Sparkscore Node instance
 */
