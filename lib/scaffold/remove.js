...
/**
 * Will remove a service from sparkscore-node.json
...
/**
 * Will remove the Node.js service and from the sparkscore-node configuration.
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The sparkscore-node configuration directory
...
function remove(options, done) {
  ...
  var sparkscoreConfigPath = path.resolve(configPath, 'sparkscore-node.json');
  var packagePath = path.resolve(configPath, 'package.json');

  if (!fs.existsSync(sparkscoreConfigPath) || !fs.existsSync(packagePath)) {
    return done(
      new Error('Directory does not have a sparkscore-node.json and/or package.json file.')
    );
  }
  ...
  async.eachSeries(
    ...
    function(service, next) {
      ...
      removeService(configPath, service, function(err) {
        ...
        // remove service to sparkscore-node.json
        removeConfig(sparkscoreConfigPath, service, next);
      });
    }, done
  );
}
