...
/**
 * Will return the path and sparkscore-node configuration
...
function findConfig(cwd) {
  ...
  while (!fs.existsSync(path.resolve(directory, 'sparkscore-node.json'))) {
  ...
  return {
    path: directory,
    config: require(path.resolve(directory, 'sparkscore-node.json'))
  };
}
