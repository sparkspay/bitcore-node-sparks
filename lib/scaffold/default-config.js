...
/**
 * Will return the path and default sparkscore-node configuration. It will search for the
 * configuration file in the "~/.sparkscore" directory, and if it doesn't exist, it will create one
...
function getDefaultConfig(options) {
  ...
  var defaultPath = path.resolve(process.env.HOME, './.sparkscore');
  var defaultConfigFile = path.resolve(defaultPath, './sparkscore-node.json');

  if (!fs.existsSync(defaultPath)) {
    mkdirp.sync(defaultPath);
  }

  var defaultServices = ['sparksd', 'web'];
