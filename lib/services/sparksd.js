...
var Proposal = sparkscore.GovObject.Proposal;
...
/**
 * Provides a friendly event driven API to sparksd in Node.js. Manages starting and
 * stopping sparksd as a child process for application support, as well as connecting
 * to multiple sparksd processes for server infrastructure. Results are cached in an
...
util.inherits(Sparks, Service);

Sparks.dependencies = [];

Sparks.DEFAULT_MAX_TXIDS = 1000;
Sparks.DEFAULT_MAX_HISTORY = 50;
Sparks.DEFAULT_SHUTDOWN_TIMEOUT = 15000;
Sparks.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS = 0.9999;
Sparks.DEFAULT_MAX_ADDRESSES_QUERY = 10000;
Sparks.DEFAULT_SPAWN_RESTART_TIME = 5000;
Sparks.DEFAULT_SPAWN_STOP_TIME = 10000;
Sparks.DEFAULT_TRY_ALL_INTERVAL = 1000;
Sparks.DEFAULT_REINDEX_INTERVAL = 10000;
Sparks.DEFAULT_START_RETRY_INTERVAL = 5000;
Sparks.DEFAULT_TIP_UPDATE_INTERVAL = 15000;
Sparks.DEFAULT_TRANSACTION_CONCURRENCY = 5;
Sparks.DEFAULT_INSTANTSEND_FEE = 10000;
Sparks.DEFAULT_CONFIG_SETTINGS = {
  server: 1,
  whitelist: '127.0.0.1',
  txindex: 1,
  addressindex: 1,
  ...
  uacomment: 'sparkscore'
};

Sparks.prototype._initDefaults = function(options) {
  /* jshint maxcomplexity: 15 */

  // limits
  this.maxTxids = options.maxTxids || Sparks.DEFAULT_MAX_TXIDS;
  this.maxTransactionHistory = options.maxTransactionHistory || Sparks.DEFAULT_MAX_HISTORY;
  this.maxAddressesQuery = options.maxAddressesQuery || Sparks.DEFAULT_MAX_ADDRESSES_QUERY;
  this.shutdownTimeout = options.shutdownTimeout || Sparks.DEFAULT_SHUTDOWN_TIMEOUT;

  // spawn restart setting
  this.spawnRestartTime = options.spawnRestartTime || Sparks.DEFAULT_SPAWN_RESTART_TIME;
  this.spawnStopTime = options.spawnStopTime || Sparks.DEFAULT_SPAWN_STOP_TIME;

  // try all interval
  this.tryAllInterval = options.tryAllInterval || Sparks.DEFAULT_TRY_ALL_INTERVAL;
  this.startRetryInterval = options.startRetryInterval || Sparks.DEFAULT_START_RETRY_INTERVAL;

  // rpc limits
  this.transactionConcurrency = options.transactionConcurrency || Sparks.DEFAULT_TRANSACTION_CONCURRENCY;

  // sync progress level when zmq subscribes to events
  this.zmqSubscribeProgress = options.zmqSubscribeProgress || Sparks.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS;
};

Sparks.prototype._initCaches = function() {
  ...
  var CACHES = [
    ...
    {name:'guardiannodeList', lru:50000},
...
Sparks.prototype._initClients = function() {
  var self = this;
  this.nodes = [];
  this.nodesIndex = 0;
 ...
 */
Sparks.prototype.getAPIMethods = function() {
  var methods = [
    ...
    ['getTxOutSetInfo', this, this.getTxOutSetInfo, 0],
    ['getSpentInfo', this, this.getSpentInfo, 1],
    ['getMNList', this, this.getMNList, 1],
    ['getGNList', this, this.getGNList, 1],
 ...
 */
Sparks.prototype.getPublishEvents = function() {
  return [
    {
...
Sparks.prototype.subscribe = function(name, emitter) {
  this.subscriptions[name].push(emitter);
  log.info(emitter.remoteAddress, 'subscribe:', 'sparksd/' + name, 'total:', this.subscriptions[name].length);
};

Sparks.prototype.unsubscribe = function(name, emitter) {
  var index = this.subscriptions[name].indexOf(emitter);
  if (index > -1) {
    this.subscriptions[name].splice(index, 1);
  }
  log.info(emitter.remoteAddress, 'unsubscribe:', 'sparksd/' + name, 'total:', this.subscriptions[name].length);
};

Sparks.prototype.subscribeAddress = function(emitter, addresses) {
  var self = this;
  ...
  if(addresses){
    for(var i = 0; i < addresses.length; i++) {
      if (sparkscore.Address.isValid(addresses[i], this.node.network)) {
        addAddress(addresses[i]);
      }
    }
  }

  log.info(emitter.remoteAddress, 'subscribe:', 'sparksd/addresstxid', 'total:', _.size(this.subscriptions.address));
};

Sparks.prototype.unsubscribeAddress = function(emitter, addresses) {
  var self = this;
  if(!addresses) {
  ...
  log.info(emitter.remoteAddress, 'unsubscribe:', 'sparksd/addresstxid', 'total:', _.size(this.subscriptions.address));
};
 ...
 */
Sparks.prototype.unsubscribeAddressAll = function(emitter) {
...
Sparks.prototype._getDefaultConfig = function() {
  var config = '';
  var defaults = Sparks.DEFAULT_CONFIG_SETTINGS;
...
Sparks.prototype._parseSparksConf = function(configPath) {
  var options = {};
...
Sparks.prototype._expandRelativeDatadir = function() {
...
Sparks.prototype._loadSpawnConfiguration = function(node) {
  /* jshint maxstatements: 25 */

  $.checkArgument(this.options.spawn, 'Please specify "spawn" in sparksd config options');
  $.checkArgument(this.options.spawn.datadir, 'Please specify "spawn.datadir" in sparksd config options');
  $.checkArgument(this.options.spawn.exec, 'Please specify "spawn.exec" in sparksd config options');

  this._expandRelativeDatadir();

  var spawnOptions = this.options.spawn;
  var configPath = path.resolve(spawnOptions.datadir, './sparks.conf');

  log.info('Using Sparks config file:', configPath);
  ...
  _.extend(this.spawn.config, this._parseSparksConf(configPath));

  var networkConfigPath = this._getNetworkConfigPath();
  if (networkConfigPath && fs.existsSync(networkConfigPath)) {
    _.extend(this.spawn.config, this._parseSparksConf(networkConfigPath));
  }
...
Sparks.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    spawnConfig.txindex && spawnConfig.txindex === 1,
    '"txindex" option is required in order to use transaction query features of sparkscore-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of sparkscore-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of sparkscore-node. ' +
  ...
  $.checkState(
    ...
    '"zmqpubrawtx" and "zmqpubhashblock" are expected to the same host and port in sparks.conf'
  );

  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that sparksd is undergoing a reindex. ' +
             ...
             'of sparkscore-node services will start.');
    node._reindex = true;
  }
};

Sparks.prototype._resetCaches = function() {
  ...
  this.guardiannodeListCache.reset();
 ...
 * @private
 */
Sparks.prototype._tryAllClients = function(func, callback) {
  var self = this;
...
Sparks.prototype._wrapRPCError = function(errObj) {
  var err = new errors.RPCError(errObj.message);
  err.code = errObj.code;
  return err;
};

Sparks.prototype._initChain = function(callback) {
  var self = this;
...
Sparks.prototype._getDefaultConf = function() {
  var networkOptions = {
    rpcport: 9998
  };
  if (this.node.network === sparkscore.Networks.testnet) {
    networkOptions.rpcport = 19998;
  }
  return networkOptions;
};

Sparks.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === sparkscore.Networks.testnet) {
    networkPath = 'testnet3/sparks.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/sparks.conf';
    }
  }
  return networkPath;
};

Sparks.prototype._getNetworkOption = function() {
  var networkOption;
  if (this.node.network === sparkscore.Networks.testnet) {
...
Sparks.prototype._zmqBlockHandler = function(node, message) {
  var self = this;
  ...
  if (!self.zmqKnownBlocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.hashblock.length; i++) {
      this.subscriptions.hashblock[i].emit('sparksd/hashblock', message.toString('hex'));
    }
  }

};

Sparks.prototype._rapidProtectedUpdateTip = function(node, message) {
  var self = this;
...
Sparks.prototype._updateTip = function(node, message) {
  var self = this;
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('Sparks Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
...
Sparks.prototype._getAddressesFromTransaction = function(transaction) {
  var addresses = [];
...
Sparks.prototype._notifyAddressTxidSubscribers = function(txid, transaction) {
  ...
  for (var i = 0; i < addresses.length; i++) {
    ...
    if(this.subscriptions.address[address]) {
      ...
      for(var j = 0; j < emitters.length; j++) {
        emitters[j].emit('sparksd/addresstxid', {
          address: address,
          txid: txid
        });
      }
...
Sparks.prototype._zmqTransactionHandler = function(node, message) {
  var self = this;
  var hash = sparkscore.crypto.Hash.sha256sha256(message);
  ...
  if (!self.zmqKnownTransactions.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.rawtransaction.length; i++) {
      this.subscriptions.rawtransaction[i].emit('sparksd/rawtransaction', message.toString('hex'));
    }

    var tx = sparkscore.Transaction();
    tx.fromString(message);
    var txid = sparkscore.util.buffer.reverse(hash).toString('hex');
    self._notifyAddressTxidSubscribers(txid, tx);

  }
};

Sparks.prototype._zmqTransactionLockHandler = function(node, message) {
  var self = this;
  var hash = sparkscore.crypto.Hash.sha256sha256(message);
  var id = hash.toString('binary');
  if (!self.zmqKnownTransactionLocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.transactionlock.length; i++) {
      this.subscriptions.transactionlock[i].emit('sparksd/transactionlock', message.toString('hex'));
    }

  }
};

Sparks.prototype._checkSyncedAndSubscribeZmqEvents = function(node) {
  var self = this;
  var interval;
  ...
  checkAndSubscribe(function(err, synced) {
    ...
    if (!synced) {
      ...
      }, node._tipUpdateInterval || Sparks.DEFAULT_TIP_UPDATE_INTERVAL);
    }
  });

};

Sparks.prototype._subscribeZmqEvents = function(node) {
  var self = this;
...
Sparks.prototype._initZmqSubSocket = function(node, zmqUrl) {
  var self = this;
...
Sparks.prototype._checkReindex = function(node, callback) {
  var self = this;
  var interval;
  function finish(err) {
    clearInterval(interval);
  ...
  interval = setInterval(function() {
    node.client.getBlockchainInfo(function(err, response) {
      ...
      log.info('Sparks Core Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
  ...
  }, node._reindexWait || Sparks.DEFAULT_REINDEX_INTERVAL);

};

Sparks.prototype._loadTipFromNode = function(node, callback) {
  var self = this;
  node.client.getBestBlockHash(function(err, response) {
    if (err && err.code === -28) {
...
Sparks.prototype._stopSpawnedSparks = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/sparksd.pid';
  ...
  function stopProcess() {
    fs.readFile(pidPath, 'utf8', function(err, pid) {
      ...
      try {
        log.warn('Stopping existing spawned sparks process with pid: ' + pid);
        self._process.kill(pid, 'SIGINT');
      } catch(err) {
        if (err && err.code === 'ESRCH') {
          log.warn('Unclean sparks process shutdown, process not found with pid: ' + pid);
          return callback(null);
        } else if(err) {
          return callback(err);
        }
...
Sparks.prototype._spawnChildProcess = function(callback) {
  var self = this;
  ...
  self._stopSpawnedSparks(function(err) {
    if (err) {
      return callback(err);
    }

    log.info('Starting sparks process');
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        log.warn('Sparks process unexpectedly exited with code:', code);
        log.warn('Restarting sparks child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            if (err) {
              return self.emit('error', err);
            }
            log.warn('Sparks process restarted');
          });
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      if (exitShutdown) {
        return callback(new Error('Stopping while trying to spawn sparksd.'));
      }
...
Sparks.prototype._connectProcess = function(config, callback) {
  var self = this;
  var node = {};
  var exitShutdown = false;
  ...
  async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
    ...
    if (exitShutdown) {
      return callback(new Error('Stopping while trying to connect to sparksd.'));
    }
 ...
 */
Sparks.prototype.start = function(callback) {
  var self = this;
  ...
  async.series([
    ...
    if (self.nodes.length === 0) {
      return callback(new Error('Sparks configuration options "spawn" or "connect" are expected'));
    }
 ...
 * @param {Function} callback
 */
Sparks.prototype.isSynced = function(callback) {
 ...
 * @param {Function} callback
 */
Sparks.prototype.syncPercentage = function(callback) {
  var self = this;
...
Sparks.prototype._normalizeAddressArg = function(addressArg) {
 ...
 * @param {String|Address|Array} addressArg - An address string, sparkscore address, or array of addresses
 * @param {Object} options
 * @param {Function} callback
 */
Sparks.prototype.getAddressBalance = function(addressArg, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Sparks.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
  var self = this;
  ...
  function transformUnspentOutput(delta) {
    var script = sparkscore.Script.fromAddress(delta.address);
    return {
...
Sparks.prototype._getBalanceFromMempool = function(deltas) {
  var satoshis = 0;
...
Sparks.prototype._getTxidsFromMempool = function(deltas) {
  var mempoolTxids = [];
...
Sparks.prototype._getHeightRangeQuery = function(options, clone) {
  if (options.start >= 0 && options.end >= 0) {
    if (options.end > options.start) {
      throw new TypeError('"end" is expected to be less than or equal to "start"');
    }
    if (clone) {
      // reverse start and end as the order in sparkscore is most recent to less recent
 ...
 * @param {Function} callback
 */
Sparks.prototype.getAddressTxids = function(addressArg, options, callback) {
...
Sparks.prototype._getConfirmationsDetail = function(transaction) {
...
Sparks.prototype._getAddressDetailsForInput = function(input, inputIndex, result, addressStrings) {
  if (!input.address) {
    return;
  }
...
Sparks.prototype._getAddressDetailsForOutput = function(output, outputIndex, result, addressStrings) {
  if (!output.address) {
    return;
  }
...
Sparks.prototype._getAddressDetailsForTransaction = function(transaction, addressStrings) {
  var result = {
    addresses: {},
    satoshis: 0
  };
 ...
 * @param {Object} txid - A sparks transaction id
 * @param {Function} callback
 */
Sparks.prototype._getAddressDetailedTransaction = function(txid, options, next) {
  var self = this;
...
Sparks.prototype._getAddressStrings = function(addresses) {
  var addressStrings = [];
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
    if (address instanceof sparkscore.Address) {
...
Sparks.prototype._paginateTxids = function(fullTxids, fromArg, toArg) {
  var txids;
 ...
 * @param {Function} callback
 */
Sparks.prototype.getAddressHistory = function(addressArg, options, callback) {
  var self = this;
  var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
Sparks.prototype.getAddressSummary = function(addressArg, options, callback) {
  var self = this;
  var summary = {};
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var summaryTxids = [];
 ...
 * @private
 */
Sparks.prototype._maybeGetBlockHash = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Sparks.prototype.getRawBlock = function(blockArg, callback) {
 ...
 * @param {Function} callback
 */
Sparks.prototype.getBlockOverview = function(blockArg, callback) {
  var self = this;
...
/**
 * Will retrieve a block as a Sparkscore object
 * @param {String|Number} block - A block hash or block height number
 * @param {Function} callback
 */
Sparks.prototype.getBlock = function(blockArg, callback) {
  // TODO apply performance patch to the RPC method for raw data
  var self = this;
    ...
    if (cachedBlock) {
      ...
      self._tryAllClients(function(client, done) {
        client.getBlock(blockhash, false, function(err, response) {
          ...
          var blockObj = sparkscore.Block.fromString(response.result);
 ...
 * @param {Function} callback
 */
Sparks.prototype.getBlockHashesByTimestamp = function(high, low, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Sparks.prototype.getBlockHeader = function(blockArg, callback) {
  var self = this;
 ...
 */
Sparks.prototype.getBlockHeaders = function(blockArg, callback, nbOfBlockToFetch) {
	var self = this;
	var _toFetch = 25;
 ...
 * @param {Function} callback
 */
Sparks.prototype.estimateFee = function(blocks, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Sparks.prototype.sendTransaction = function(tx, options, callback) {
  var self = this;
...
Sparks.prototype._getRawTransactionData = function(txid, callback) {
  var self = this;
  self._tryAllClients(function(client, done) {
 ...
 * @param {Function} callback
 */
Sparks.prototype.getRawTransaction = function(txid, callback) {
  var self = this;
...
/**
 * Will get a transaction as a Sparkscore Transaction. Results include the mempool.
 * @param {String} txid - The transaction hash
 * @param {Boolean} queryMempool - Include the mempool
 * @param {Function} callback
 */
Sparks.prototype.getTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Sparks.prototype.getDetailedTransaction = function(txid, callback) {
  var self = this;
  ...
  function addTxlockToTx(tx,result) {
    tx.txlock = false;
    if (tx.feeSatoshis >= Sparks.DEFAULT_INSTANTSEND_FEE) { // if transaction fee is >= 0.0001 check for txlock
      var rawTx = new sparkscore.Transaction(result.hex);
      var hash = sparkscore.crypto.Hash.sha256sha256(rawTx.toBuffer());
      var id = hash.toString('binary');
      if (self.node.services.sparksd.zmqKnownTransactionLocks.get(id)) {
 ...
 * @param callback
 */
Sparks.prototype.govObjectList = function(options, callback) {
  var self = this;
...
Sparks.prototype.getCurrentVotes = function (govhash, callback) {
  var self = this;
...
Sparks.prototype.getVotes = function (govhash, callback) {
  var self = this;
...
Sparks.prototype.getSuperBlockBudget = function (blockindex, callback) {
  var self = this;
...
/*jshint maxparams: 6 */
Sparks.prototype.govObjectSubmit = function (parentHash, revision, time, dataHex, feeTxId, callback) {
  var self = this;
...
Sparks.prototype.govObjectDeserialize = function (hexdata, callback) {
  var self = this;
...
Sparks.prototype.govObjectCheck = function(hexdata, callback){
  var self = this;
...
Sparks.prototype.govObjectInfo = function (callback) {
  var self = this;
...
Sparks.prototype.govCount = function (callback) {
  var self = this;
...
Sparks.prototype.getSpork = function(callback){
	var self = this;
	var SporksList = {};
...
Sparks.prototype.getMNList = function(callback){
  var self = this;
  var rawResults= {};
  var MNList = [];
...
Sparks.prototype.getGNList = function(callback){
  var self = this;
  var rawResults= {};
  var GNList = [];
  ...
  var getRank = function(next){
    self.client.guardianlist('rank', function(err, response){
      if(response && response.hasOwnProperty('result')){
        var result = response.result;
        rawResults.rank=result;
      }
  ...
  var getProtocol = function(next){
	  self.client.guardianlist('protocol', function(err, response){
		  if(response && response.hasOwnProperty('result')){
			  var result = response.result;
			  rawResults.protocol=result;
		  }
  ...
  var getPayee = function(next){
	  self.client.guardianlist('payee', function(err, response){
		  if(response && response.hasOwnProperty('result')){
			  var result = response.result;
			  rawResults.payee=result;
		  }
  ...
  var getLastSeen = function(next){
	  self.client.guardianlist('lastseen', function(err, response){
		  if(response && response.hasOwnProperty('result')){
			  var result = response.result;
			  rawResults.lastseen=result;
		  }
  ...
  var getActiveSeconds=function(next){
	  self.client.guardianlist('activeseconds', function(err, response){
		  if(response && response.hasOwnProperty('result')){
			  var result = response.result;
			  rawResults.activeseconds=result;
		  }
  ...
  var getIP = function(next){
	  self.client.guardianlist('addr', function(err, response){
		  if(response && response.hasOwnProperty('result')){
			  var result = response.result;
			  rawResults.addr=result;
		  }
  ...
  var getStatus = function(next){
	  self.client.guardianlist('status', function(err, response){
		  if(response && response.hasOwnProperty('result')){
			  var result = response.result;
			  rawResults.status=result;
		  }
  ...
  var prepareResponse = function(err){
    ...
    if(
        ...
        for(var i = 0; i<rankLength; i++){
          ...
          return callback(new Error('Invalid GuardiannodeList'),null);
    }
	  self.guardiannodeListCache.set('', GNList);
	  return callback(null, GNList);
  };
  var GNListData = self.guardiannodeListCache.get('');
  if (GNListData) {
      return setImmediate(function() {
	      callback(null, GNListData);
      });
  } else {
	  return async.series([
	    checkSync,
 ...
 * @param callback
 */
Sparks.prototype.govObjectHash = function(hash, callback) {
  var self = this;
 ...
 * @param callback
 */
Sparks.prototype.govObjectCheck = function govObjectCheck(hexdata, callback) {
  var self = this;
  this.client.gobject('check', hexdata, function (err, response) {
    if (err) {
      return callback(self._wrapRPCError(err));
 ...
 * @param {Function} callback
 */
Sparks.prototype.getBestBlockHash = function(callback) {
  var self = this;
...
/**
 * Will get the best currentnetworkHashPs for the chain.
 * @param {Function} callback
 */
Sparks.prototype.getNetworkHashps = function(callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Sparks.prototype.getTxOutSetInfo = function(callback) {
  var self = this;
  this.client.getTxOutSetInfo(function(err, response) {
    if (err) {
      return callback(self._wrapRPCError(err));
    }
    callback(null, response.result);
 ...
 * @param {Function} callback
 */
Sparks.prototype.getSpentInfo = function(options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Sparks.prototype.getInfo = function(callback) {
  var self = this;
...
Sparks.prototype.generateBlock = function(num, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Sparks.prototype.stop = function(callback) {
  if (this.spawn && this.spawn.process) {
    ...
    this.spawn.process.once('exit', function(code) {
      if (!exited) {
        exited = true;
        if (code !== 0) {
          var error = new Error('sparksd spawned process exited with status code: ' + code);
          error.code = code;
    ...
    setTimeout(function() {
      if (!exited) {
        exited = true;
        return callback(new Error('sparksd process did not exit'));
      }
